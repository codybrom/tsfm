...
Captured 2026-09-18 on macOS 27.0 (26A428), right after running `sudo fm license`. See `unlicensed.md` for the state before agreeing.
`licensed.json` holds the CLI runs, recorded with `probe.ts`. The `fm serve` results below were
recorded by hand with `curl --unix-socket`.
...
- `fm license --status` → exit **0**, `Agreed to license FM1 version 1.0 on <localized date>.`
  ...
  In `licensed.json` the date and time are redacted as `<date> at <time>`; the U+202F before AM/PM is kept.
