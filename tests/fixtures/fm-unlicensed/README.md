...
Each directory holds one `fm` run: `cmd.txt`, `stdin_mode.txt` (`null` = `</dev/null`,
`pipe` = `echo no |`, `tty` = pty via `script(1)`), `stdout.txt`, `stderr.txt`, `exit.txt`,
`seconds.txt`, and `tty.txt` for pty runs. `../fm-harness/probe.sh` made them (run with `EXPECT_UNLICENSED=1`).
