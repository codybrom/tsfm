...
// On 0.x the bridge header is copied into native/ by the native build, so the
// sync checks run only once it exists.
const headerFiles = ["native/FoundationModels.h", "native/extensions/TsfmExtensions.h"];
const haveHeaders = headerFiles.every((file) => existsSync(path.join(root, file)));
const header = haveHeaders
  ? headerFiles.map((file) => readFileSync(path.join(root, file), "utf8")).join("\n")
  : "";
...
describe.runIf(haveHeaders)("string parameters in the koffi signatures", () => {
