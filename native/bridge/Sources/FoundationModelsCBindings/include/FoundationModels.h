...
// `cancelled` reports a cancelled invocation by id, after its call notification.
// `releaseUserInfo` (if set) is called with it once, when the tool is freed. On
// failure (NULL), the caller still owns `userInfo`.
FMBridgedToolRef _Nullable FMBridgedToolCreateWithUserInfo(const char *_Nonnull name, const char *_Nonnull description, FMGenerationSchemaRef _Nonnull parameters, void (*_Nonnull callable)(FMGeneratedContentRef _Nonnull, unsigned int, void *_Nullable), void (*_Nullable cancelled)(unsigned int, void *_Nullable), void *_Nullable userInfo, void (*_Nullable releaseUserInfo)(void *_Nullable), int *_Nullable outErrorCode, char *_Nullable *_Nullable outErrorDescription) __attribute__((swift_attr("@Sendable")));
